...
results_file="resultados_experimento_anakena.txt"  # Archivo para guardar resultados
...
python3 create_files.py "$test_file" 512000 # 500KB para Anakena
python3 create_files.py "smallfile" 10240 50  # 50 archivos de 10 KB (total ~500 KB)
...
for chunk in "${chunk_sizes[@]}"; do
    ...
    if [ $total_expected -gt 0 ]; then
        ...
        log_result "Total esperado: $((total_expected/1024)) KB ($total_expected bytes)"
        log_result "Total recibido: $((total_received/1024)) KB ($total_received bytes)"
